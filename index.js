...
const elasticSVG = require('elastic-svg');
...
var DEBUG = {
	...
	tickOnClick: false,			// advance the simulation with `.tick()` while clicking anywhere on the hourglass
	tickToStart: false,			// whether to start the simulation right away or force one to click through it
	dontAutoStart: false,			// whether to skip the initial animation
	showConsole: false			// whether to suppress `console.log`
};

var PLAYING = false;

const SVG = require("./src/hourglass.json");
...
var hourglass = function(id, options) {
	...
	// make SVG and populate
	var base = elasticSVG(id, {
		width: 320,
		aspect: 420 / 320,
		resize: "auto"
	});

	var svg = select(base.svg)
		.classed("hourglass", true)
		.html(SVG.hourglass);

	var sandbox = svg.append("g").attr("id", "sandbox");

	// generate the node objects
	var nodes = range(options.count).map(function(i) {
		var node = {
			id: i,
			r: options.radius,
	...
	var x = forceX(TARGETS.bottom.x).strength(0.012);
	var y = forceY(TARGETS.bottom.y).strength(function(d) {
		if (d.confined) {
	...
	var simulation = forceSimulation(nodes)
		...
		.force("collide", forceCollide(options.radius + 1).iterations(3))
	...
	function ticked() {
		...
		grains.each(function(d, i, t) {
			...
			d.isInBounds = polygonContains(polygon, point);
	...
	function isInsideGlass(alpha) {
		...
		for (var i = 0, n = nodes.length, node; i < n; ++i) {
			...
			node.isInBounds = polygonContains(polygon, point);
